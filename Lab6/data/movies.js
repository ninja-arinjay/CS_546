...
const getMovieById = async (movieId) => {
  ...
  const moviesCollection = await movies();
  const idMovie = await moviesCollection.findOne({_id: ObjectId(movieId)});
...
const updateMovie = async (
  ...
  //Input Validation of castMembers
  helpers.checkInputArray(castMembers);
  for (i in castMembers) {
    if (typeof castMembers[i] !== 'string' || castMembers[i].trim().length === 0) {
      castMembersInvalidFlag = true;
  ...
  //Input Validation of dateReleased
  helpers.checkInputString(dateReleased);
  dateReleased = dateReleased.trim();
  helpers.checkDate(dateReleased);
  ...
  const moviesCollection = await movies();
  const updatedInfo = await moviesCollection.findOne(
    {_id : ObjectId(movieId)}
  );
  if(!updatedInfo){
    throw "ERROR: NO Movie Is Present For That ID";
  };
  let moviesUpdateInfo = {
    title : title,
    plot : plot,
    genres : genres,
    rating : rating,
    studio : studio,
    director : updatedInfo.director,
    castMembers : castMembers,
    dateReleased : dateReleased,
    runtime : runtime,
    reviews : updatedInfo.reviews,
    overallRating : updatedInfo.overallRating
  };
  const updateI = await moviesCollection.updateOne(
    {_id: ObjectId(movieId)},
    {$set: moviesUpdateInfo}
  ) 
